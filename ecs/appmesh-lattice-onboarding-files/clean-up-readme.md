...
## Step 2: Clean up Lambda function

```bash
aws lambda delete-function --function-name frontend-ui-test --region us-west-2
```
...
aws logs delete-log-group --log-group-name "/aws/lambda/frontend-ui-test" --region us-west-2
```

## Step 4: Clean up App Mesh Resources
...
## Step 5: Clean up VPC Lattice resources
...
- Delete Lattice Target Group (works after ECS service `product-lattice-service` is fully deleted)
...
## Step 7: Clean up database artifacts used by the application
...
IAM Roles for VPC Lattice, Lambda Execution, ECS access to invoke Lambda function, ECS role to update ALB listener weights, ECS Task and ECS Task Execution
